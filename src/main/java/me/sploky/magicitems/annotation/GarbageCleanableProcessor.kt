...
class GarbageCleanableProcessor: AbstractProcessor() {
    companion object {
        const val KAPT_KOTLIN_GENERATED_OPTION_NAME = "kapt.kotlin.generated"
    }

    override fun process(annotations: MutableSet<out TypeElement>?, roundEnv: RoundEnvironment?): Boolean {
        roundEnv?.getElementsAnnotatedWith(GarbageCleanable::class.java)
            ?.forEach {
    ...
    override fun getSupportedAnnotationTypes(): MutableSet<String> {
        return mutableSetOf(GarbageCleanable::class.java.name)
    }
        ...
        for (enclosed in element.enclosedElements) {
            if (enclosed.kind == ElementKind.FIELD) {
                classBuilder.addProperty(
                    PropertySpec.builder(enclosed.simpleName.toString(), enclosed.asType().asTypeName().asNullable(), KModifier.PRIVATE)
                        .initializer("null")
                        .build()
                )
